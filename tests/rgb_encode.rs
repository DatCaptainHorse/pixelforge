...
/// The hardware conversion's levels, measured absolutely rather than against
/// the shader.
///
/// Comparing the two paths cannot catch a range error they share, and a range
/// error is exactly what a driver ignoring its range setting produces: a flat
/// grey that should decode to one code value decodes to another, and the
/// picture as a whole comes out washed out or crushed.
#[test]
#[ignore = "requires a Vulkan Video device with VK_VALVE_video_encode_rgb_conversion"]
fn rgb_input_writes_limited_range_levels() -> Result<(), Box<dyn std::error::Error>> {
    let context = context(Codec::H264)?;
    let Some(format) = rgb_format(&context, "rgb_input_writes_limited_range_levels") else {
        return Ok(());
    };
    let mut encoder = Encoder::new(
        context.clone(),
        encode_config(Codec::H264).with_rgb_input(format),
    )?;
    let grey: Vec<u8> = (0..WIDTH * HEIGHT)
        .flat_map(|_| [51u8, 51, 51, 255])
        .collect();
    let src = unsafe { create_src_image(&context, WIDTH, HEIGHT, &grey)? };
    let mut stream = Vec::new();
    for _ in 0..FRAMES {
        stream.extend_from_slice(&pollster::block_on(encoder.encode(src.image)?)?.data);
    }
    let frames = decode_all(&context, &stream)?;
    let last = frames.last().ok_or("no frames decoded")?;
    let mean = last.iter().map(|&v| f64::from(v)).sum::<f64>() / last.len() as f64;
    // Limited range maps 0..255 onto 16..235: 16 + 51 * 219 / 255.
    let expected = 16.0 + 51.0 * 219.0 / 255.0;
    assert!(
        (mean - expected).abs() <= 1.0,
        "flat grey 51 decoded to luma {mean:.1}, limited range puts it at {expected:.1}"
    );
    Ok(())
}

/// A full-range conversion is never handed to the hardware, since the one
/// driver that offers the path writes limited range regardless.
#[test]
#[ignore = "requires a Vulkan Video device"]
fn full_range_is_never_offered_to_the_hardware() -> Result<(), Box<dyn std::error::Error>> {
    let context = context(Codec::H264)?;
    let mut config = converter_config();
    config.range = ColorRange::Full;
    assert_eq!(config.rgb_encode_input(&context), None);
    let full = encode_config(Codec::H264)
        .with_color_description(config.color_description().unwrap())
        .with_rgb_input(InputFormat::BGRA);
    if context.has_video_encode_rgb_conversion() {
        assert!(Encoder::new(context.clone(), full).is_err());
    }
    Ok(())
}
